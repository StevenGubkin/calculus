...
\end{explanation}
\end{example}
>>>>>>> 524bb97b72e471e81e9434d86f7330cf195a18bf
...
Compute:
\[
\lim_{x\to 2^-} f(x) = \begin{prompt} \answer{1}\end{prompt}
\]

\[
\lim_{x \to 2^+} f(x) = \begin{prompt} \answer{2}\end{prompt}
\]
...
\begin{question}
	...
	\begin{question}
			On the following coordinate grid, sketch the graph of a function enjoying the following properties.
			
			\begin{itemize}
					\item $f(-10)=10$
					\item $\displaystyle \lim_{x \to -7^-} f(x) = 5$
					\item $\displaystyle \lim_{x \to -7^+} f(x) = f(-7) = 2$
					\item $\displaystyle \lim_{x \to 0} f(x) = 3$
					\item $\displaystyle f(0) = 4$
					\item $\displaystyle \lim_{x \to 3} = -1$
					\item $f$ is not defined at $x=3$
					\item $f(10) = 0$
				\end{itemize}
			...
			\begin{image}
				...
				\begin{tikzpicture}
				\begin{axis}[
				domain=-10:10, xmin=-10, xmax=10, ymin=-5,ymax=10,    
				unit vector ratio*=1 1 1,
				axis lines =middle, xlabel=$x$, ylabel=$y$,
				every axis y label/.style={at=(current axis.above origin),anchor=south},
				every axis x label/.style={at=(current axis.right of origin),anchor=west},
				xtick={-10,...,10}, ytick={-5,...,10},
				xticklabels={-10,,-8,,-6,,-4,,-2,,0,,2,,4,,6,,8,,10}, yticklabels={,-4,,-2,,0,,2,,4,,6,,8,,10},
				grid=major,width=8in,
				grid style={dashed, gridColor},
				]
